...
var getNetworkData = function(errors) {
	var datas = [];
    var ifaces = shell.ls('/sys/class/net');
    var ifaces2 = ifaces.stdout.split('\n');
    console.error(ifaces);
//	var ifaces = os.networkInterfaces();
	ifaces2.forEach( function  (iface) { 
        if ((iface.indexOf(':') > -1) || 
            (iface.indexOf('lo') > -1) || 
            (iface.indexOf('vlan') > -1) ||
            (iface.indexOf('veth') > -1) ||
            (iface.indexOf('vnet') > -1) ||
            (iface.trim().length < 2)
        ) {
            return;
        }
		...
		try {
            info.ip = fs.readFileSync('/sys/class/net/' + iface + '/address').toString().trim();
...
var getDiskData = function(errors) {
	try {
		var df = exec('/bin/df -T | awk -v c=`/bin/df -T | grep -bo "Type" | awk -F: \'{print $2}\'` \'{print substr($0,c);}\' | tail -n +2 | awk \'{print $1","$2","$3","$4","$5","$6","$7}\'').toString();
	} catch(err) {
	...
	var mounts = [];
	var key = 0;
	var parsed = df.trim().split('\n');
	for(var item in parsed) {
		var it = parsed[item];
		if(it.indexOf('tmpfs') > -1 || it.indexOf("overlay") > -1) {
			continue;
		}
		...
		datas.push({
			...
			"percent_used": it[5],
			mount: it[6].trim(),
			filesystem: it[0].replace("/dev/mapper/", "").replace("/dev/", "").trim()
		})
	}
	return datas;
}
